...
def ping(host, mascara):
    ...
    def pacote_do_ping(ip, ipsAcessiveis, lock):
        ...
        try:
            ...
            if f'Resposta de {ip}' in resultados.stdout or f'64 bytes from {ip}' in resultados.stdout:
                with lock:
                    ipsAcessiveis.append(ip)
        except:
            pass


    def criando_pool_de_threads(listaDeIps, ipsAcessiveis, lock):
        poolDeThreads = list()
        for ip in listaDeIps:
            thread = threading.Thread(target=pacote_do_ping, args=(ip, ipsAcessiveis, lock,))
    ...
    poolDeThreads = criando_pool_de_threads(listaDeIps, ipsAcessiveis, lock)
    executando_threads(poolDeThreads)

    ipsAcessiveis = sorted(ipsAcessiveis)
    return ipsAcessiveis
